...
class NotificationProcess(plugin_base.NotificationBase):
    ...
    def get_targets(self, conf):
        return [oslo_messaging.Target(topic=self.topic, exchange=conf.ceilometer_control_exchange)]
...
class NotificationEndpoint(object):
    ...
    def process_notification(self, message):
        # XXX this is where some processing will take place, this is just a placeholder
...
class NotificationServiceProcessor(cotyledon.Service):
    ...
    def run(self):
        ...
        handler = NotificationProcess(NotificationManager(self.conf), self.topic)
        endpoints = [handler]
        targets = handler.get_targets(self.conf)
        """

        endpoints = [NotificationEndpoint()]
        targets = [oslo_messaging.Target(topic=self.topic, exchange=self.conf.ceilometer_control_exchange)]
